 ...
 *
 * Likes go through the `toggle_recipe_like` RPC rather than writing `recipe_likes` and
 * `custom_recipes.likes_count` from here: the old client-side read-modify-write lost
 * concurrent likes and let any client set the counter to any value.
 */
...
export default function Lounge() {
    ...
    const toggleLike = async (e: React.MouseEvent, recipeId: string) => {
        ...
        if (result) {
            ...
            setRecipes(prev => prev.map(r =>
                r.id === recipeId ? { ...r, likes_count: result.likes_count } : r
            ));
        }
    };
                            ...
                            <div key={recipe.id} className="relative group">
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <div className="flex items-center justify-between gap-3">
                                                ...
                                                <span className="text-zinc-400 text-[9px] uppercase font-bold tracking-widest truncate max-w-[45%]">
                                                    by {recipe.profiles?.username || 'a mixologist'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="absolute top-6 right-6 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                    ...
                                    <button
                                        ...
                                        aria-label={`View ${recipe.profiles?.username || 'creator'}'s profile`}
                                        className="p-3 rounded-full bg-black/60 border border-white/10 text-zinc-400 hover:text-primary backdrop-blur-md shadow-xl transition-all"
                                    >
                                        <User size={18} />
                                    </button>
